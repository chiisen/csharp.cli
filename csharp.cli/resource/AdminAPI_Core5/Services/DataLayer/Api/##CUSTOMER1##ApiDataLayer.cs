...
namespace APPAPI.Services.DataLayer.Api
{
    ...
    /// <returns></returns>
    public async Task<ResponseInfoModel<UrlInfoModel<string>>> GetGameToken(##CUSTOMER1##GetGameTokenRequestModel
        paramList, string clientIP)
    {
    ...
    /// <param name="paramList"></param>
    /// <returns></returns>
    public async Task<ResponseInfoModel<List<W1GetBetRecordData##CUSTOMER##>>> GetBetRecord(
        W1GetBetRecordRequestModel paramList)
    {
        var betRecordResult = await this._w1WalletDataLayer.GetBetRecord < W1GetBetRecordData##CUSTOMER##>(paramList,
            this._thirdPartyId);
