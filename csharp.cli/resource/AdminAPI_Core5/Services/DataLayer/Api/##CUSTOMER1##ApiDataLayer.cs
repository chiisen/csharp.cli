...
public class ##CUSTOMER1##ApiDataLayer : BaseApiDataLayer
{
	...
	public ##CUSTOMER1##ApiDataLayer(
		...
		: base(appSetting, webSiteSettingService, httpService, commonService, cacheService, gameDataLayer, memberDataLayer, jwtService, redisService, h1WalletDataLayer, w1WalletDataLayer, unReturnAccountDataLayer, webCacheDataLayer, iServiceProvider, logScope)
	{
		this._logger = logger;
	...
	public async Task<ResponseInfoModel<UrlInfoModel<string>>> GetGameToken(##CUSTOMER1##GetGameTokenRequestModel paramList, string clientIP)
	{
		...
		var result = await this._h1WalletDataLayer.ForwardGame(this._thirdPartyId, MaintainStatus.##CUSTOMER1##Room_StartFlag, this._appSetting.GameEntryRoom.##CUSTOMER##,
			async () =>
			{
				// 取得進線網址
				var forwardGameResult = await this._h1WalletDataLayer.ForwardGameH1WalletFunction(new W1ForwardGameRequestModel<W1ForwardGameConfig##CUSTOMER##>()
				{
